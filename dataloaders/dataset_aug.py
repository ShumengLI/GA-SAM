...
from batchgenerators.transforms.color_transforms import BrightnessMultiplicativeTransform, ContrastAugmentationTransform, GammaTransform, BrightnessTransform
from batchgenerators.augmentations.utils import create_zero_centered_coordinate_mesh, elastic_deform_coordinates, interpolate_img, rotate_coords_2d, rotate_coords_3d, scale_coords
    ...
    def __call__(self, **data_dict):
        ...
        for b in range(data.shape[0]):
            if random.random() < self.p_per_sample:
                data, label = augment_scale(data, label, patch_size=data.shape[2:], scale=self.scale)
...
class RotationTransform(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        ...
        for b in range(data.shape[0]):
            if random.random() < self.p_per_sample:
                data, label = augment_rotation(data, label, patch_size=data.shape[2:])
        data_dict[self.data_key] = data
        data_dict[self.label_key] = label
        return data_dict
...
class begin(AbstractTransform):
    def __call__(self, **data_dict):
        ...
        if 'weight' in data_dict:
            return {'image': image, 'label': label, 'weight': torch.from_numpy(data_dict['weight'])}
        elif 'weight_2d' in data_dict:
            return {'image': image, 'label': label, 'weight_2d': data_dict['weight_2d']}
        elif 'plabel' in data_dict:
            return {'image': image, 'label': label, 'plabel': torch.from_numpy(data_dict['plabel']).long()}
        else:
            return {'image': image, 'label': label}


def get_StrongAug_pixel(sample_num, p_per_sample=0.3):
    tr_transforms = []
